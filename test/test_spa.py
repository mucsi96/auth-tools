...
def test_denies_change_for_user_without_admin_role(page: Page):
    ...
    sign_in(page, "user2")
    page.get_by_role("button", name="Change").click()
    expect(page.get_by_role("status").filter(has_text="You are not allowed to change user data.")).to_be_visible()
    
def test_denies_change_if_user_is_not_signed_in(page: Page):
    page.goto("https://spa.auth-tools.home")
    page.get_by_role("button", name="Change").click()
    expect(page.get_by_role("status").filter(has_text="You are not authorized to change user data.")).to_be_visible()
